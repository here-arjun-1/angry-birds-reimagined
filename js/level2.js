...
function drawObjects() {
  for (let i = 0; i < woods.length; i++) {
    ...
    if (wood.exploding) {
      ...
      ctx.save();
      ctx.translate(wood.x + wood.width, wood.y + wood.height);
      ctx.rotate(wood.angle);
      drawWood(-wood.width, -wood.height, wood.width, wood.height);
      ctx.restore();
    }
  }
...
function drawWinScreen() {
  ...
  ctx.fillText("LEVEL 2 COMPLETE!", canvas.width / 2, canvas.height / 2 - 60);
...
function drawFailScreen() {
  ...
  ctx.textAlign = "center";
  ctx.font = "bold 60px Arial";
  ctx.fillText("LEVEL 2 FAILED!", canvas.width / 2, canvas.height / 2 - 40);
  ctx.font = "bold 35px Arial";
